...
| 4. LLM 기반 Phase / signal 판정 | 결정적 기준만 존재                 | shadow             |
| 5. diff 기반 실수 패턴 학습     | fix 흐름은 있으나 의미 학습은 약함 | guarded-shadow     |
| 6. Ack 조건 강화                | file-write 중심 ack                | guarded            |
...
현재 phase는 `phase-manager.ts`의 결정적 로직이 관리한다. signal도 현재는 규칙화 가능한 형태가 우선이다.
...
`fix_commit` 흐름과 diff 파싱 하드닝은 이미 있다. 다만 그 diff에서 "왜 고쳤는지"를 의미적으로 학습하는 단계는 아직 아니다.
...
지금은 처리 결과를 파일에 쓰고 ack로 넘기는 흐름이 중심이다. 이 방식은 너무 빨리 닫힐 수 있다.

### 의도

ack를 "저장 성공"이 아니라 실제 통과로 바꾼다.
...
- 처음에는 ack를 두 단계로 나눈다: `written` → `accepted`.
