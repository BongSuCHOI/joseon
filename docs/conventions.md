...
│   └── constants.ts             # HARNESS_DIR, HOME, THIRTY_DAYS_MS, MAX_ERROR_SUMMARY_LENGTH + Token Optimizer v0 (BUDGET_LIMITS, DANGER_PATTERNS, COMPACT_OVERRIDE_PROMPT, FILE_DEDUPER_THRESHOLD) + v0.5 (TOKEN_OPTIMIZER_METRICS_FILE, METRICS_MAX_BYTES, METRICS_RECOMMENDATION_WINDOW)
│
├── config/                      # 설정 시스템
│   ├── index.ts                 # 배럴 export
│   ├── schema.ts                # HarnessConfig, AgentOverrideConfig, HarnessSettings (낭비 탐지 임계값, Fact TTL 설정, Phase 1a 메모리 의미론 토글 5개, Token Optimizer v0 토글 5개 + v1 configurable thresholds: budget_limits_search/read/test/write/other, file_deduper_threshold, git_log_session_limit 포함)
│   └── loader.ts                # JSONC/JSON 로더 + 글로벌/프로젝트 병합
│
└── types.ts                     # 전체 타입 정의 (Signal, Rule, ProjectState, ToolCategory, DangerPattern, TokenOptimizerMetricsRecord 등)
```
