...
v0를 2주간 사용 후 다음 기준으로 v1 기능을 결정한다:

| 지표 | 측정 방법 | v1 판단 기준 |
|------|----------|------------|
| pre_tool_guard 오탐율 | 차단 후 에이전트가 대안으로 성공한 비율 | < 80%면 패턴 조정 |
| loop_budget 도달 빈도 | 세션 중 예산 소진 비율 | > 30%면 한계 상향 |
| file_deduper 오탐율 | 차단 후 파일이 실제 변경된 비율 | > 5%면 핑거프린트 개선 |
| compact_override 만족도 | 컴팩션 후 세션 연속 성공률 | < 70%면 프롬프트 개선 |
| 세션 압력 분포 | observer에 측정 전용 카운터 추가 | medium/high가 30% 이상이면 session_pressure v1 도입 |
