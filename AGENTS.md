...
| 4 | 오케스트레이션 | + orchestrator (에러 복구 5단계, 5-Phase, 서브에이전트 라우팅) | ✅ 완료 — Step 4a~4f (안정화 후속 포함, 통합 테스트 248/248 통과) |
...
| 항목 | 내용 | v3 섹션 |
|------|------|---------|
| 에러 복구 5단계 | 1차 직접수정 → 2차 구조변경 → 3차 다른 모델 rescue → 4차 리셋 | 5.2.2 |
| 5-Phase 워크플로우 | frontend → backend → tester → reviewer. Phase 간 의존성 관리 | 7장 |
...
| 항목 | 내용 | 비고 |
|------|------|------|
| foreground-fallback | same-session reactive fallback 훅 | rate limit 등 retryable 실패 시 현재 세션을 abort하고 다음 모델로 재프롬프트 |
| filter-available-skills | 에이전트별 스킬 필터 훅 | available_skills 노출을 에이전트별 skills로 제한 |
...
**구현/질의응답/단순질문 혼재 — 최상위 Orchestrator 단일 라우터 (확정):**
...
Orchestrator (최상위, 기본 에이전트)
  ...
  ├── 판단: 대화/단순 조회인가?
  │   ├── Yes → 직접 처리
  │   └── No  → 적절한 specialist에게 위임
  └── Phase 관여는 오케스트레이션 인프라가 담당
```

근거:
- 관심사 분리: Orchestrator는 판단/라우팅, specialist는 구현/검증/조사. 각 에이전트 프롬프트가 짧고 명확
...
- frontend — UI 구현 전문
- backend — 서버/비즈니스 로직 전문
- tester — 테스트 작성/실행 전문
- reviewer — 읽기 전용 코드 리뷰
- designer — UI/UX 아이디어와 DESIGN.md 작성
- explorer — 읽기 전용 코드베이스 탐색
- librarian — 읽기 전용 외부 문서 조사
- coder — 기계적 편집 전문
- advisor — 읽기 전용 전략 자문

해결된 포인트:
- **작업 중간 전환:** tab으로 Orchestrator 전환 → Phase 파일 무시하고 답변 → tab으로 다시 돌아오면 현재 상태를 이어서 처리
- **버그 수정 라우팅:** Orchestrator가 판단. 작은 수정은 독립 서브에이전트에게 직접 위임, 큰 수정은 적절한 specialist 조합으로 분배
- **중간 포기:** Phase 파일 유지. 다음 Orchestrator 호출 시 미완료 Phase 감지하여 사용자에게 이어서/새로 시작 질문
...
| **hooks 보강** (`src/hooks/`) | ✅ 완료 | delegate-task-retry, json-error-recovery, post-file-tool-nudge, post-read-nudge, phase-reminder, foreground-fallback, filter-available-skills (7개) | `hooks/` 15개 |
...
│   ├── orchestrator.ts          # Plugin 4: session.idle Phase 정리
│   ├── phase-manager.ts         # Phase 상태 파일 관리 + Phase 2.5 gate + PID 세션 락
│   ├── error-recovery.ts        # 에러 복구 5단계 에스컬레이션
│   ├── qa-tracker.ts            # QA 시나리오별 실패 추적 + 반복 검출
...
│       ├── advisor.md           # advisor 서브에이전트 (심층 분석, 아키텍처 자문)
...
│   ├── foreground-fallback.ts   # same-session reactive fallback 훅
│   └── filter-available-skills.ts # 에이전트별 스킬 노출 필터
│
├── shared/                      # 공통 유틸리티
│   ├── index.ts                 # 배럴 export
│   ├── utils.ts                 # getProjectKey, ensureHarnessDirs, generateId, parseList, mergeEventHandlers, rotateHistoryIfNeeded, isPluginSource
...
export default {
  ...
  server: async (input) => {
    ...
    (result as Record<string, unknown>).config = async (opencodeConfig) => {
      const agents = createAgents(harnessConfig);
      // 에이전트별 MCP/Skills/Tool deny permission 자동 생성 + 등록
      // buildMcpPermissions, buildSkillPermissions, buildToolPermissions
    };
