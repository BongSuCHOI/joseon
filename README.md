...
| **improver** | `src/harness/improver.ts` | L5 signal→규칙 변환 + fix: 커밋 학습 + L6 승격/효과측정 + compacting 컨텍스트 주입 |
...
│   ├── utils.ts                 # getProjectKey, ensureHarnessDirs, logEvent, mergeEventHandlers
│   └── index.ts                 # 배럴 export
└── harness/
